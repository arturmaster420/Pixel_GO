...
export class Player {
  constructor(startPos, startLevel) {
    ...
    this.energyBombCooldown = 0;
    this.fireBombCooldown = 0;
    this.iceBombCooldown = 0;
  ...
  update(dt, state) {
    ...
    // Legacy world clamp only applies before the room/floor system is ready.
    // In Pixel GO floors, final collision is handled by floorCollision.js in the main loop.
    if (!state?.roomDirector?.current) {
