...
export function drawAura(ctx, x, y, baseR, nowSec, idSeed, auraId = 0, dead = false, lowFx = false) {
  ...
  // Rotating rings (adds obvious "motion" so aura feels more alive)
  {
    const rot = nowSec * 0.95 + phase;
    const ringR = outer * 0.78;
    const ringR2 = outer * 0.95;
    ctx.lineCap = 'round';
    ctx.lineWidth = lowFx ? 2.2 : 3.0;
    ctx.strokeStyle = `rgba(${pal.b[0]},${pal.b[1]},${pal.b[2]},${lowFx ? 0.12 : 0.18})`;
    ctx.beginPath();
    ctx.arc(x, y, ringR, rot, rot + Math.PI * 0.85);
    ctx.stroke();
    ctx.strokeStyle = `rgba(${pal.a[0]},${pal.a[1]},${pal.a[2]},${lowFx ? 0.10 : 0.16})`;
    ctx.beginPath();
    ctx.arc(x, y, ringR2, -rot * 0.9, -rot * 0.9 + Math.PI * 0.62);
    ctx.stroke();

    // Small orbiting spark
    const sx = x + Math.cos(rot * 1.25) * ringR2;
    const sy = y + Math.sin(rot * 1.25) * ringR2;
    const sg = ctx.createRadialGradient(sx, sy, 0, sx, sy, outer * 0.18);
    sg.addColorStop(0, 'rgba(255,255,255,0.18)');
    sg.addColorStop(0.4, `rgba(${pal.a[0]},${pal.a[1]},${pal.a[2]},0.10)`);
    sg.addColorStop(1, `rgba(${pal.c[0]},${pal.c[1]},${pal.c[2]},0)`);
    ctx.fillStyle = sg;
    ctx.beginPath();
    ctx.arc(sx, sy, outer * 0.18, 0, Math.PI * 2);
    ctx.fill();
  }
