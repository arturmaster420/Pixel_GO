...
export function offerNeedsReplace(player, offer) {
  ...
  if (!isLiveSkillUnlockOffer(player, offer)) return false;
...
export function tryBuyFloorShopOfferEx(player, offer, replaceKey) {
  ...
  // Enforce max active skills using the player's live state at click time
  // (not the stale offer snapshot from when the terminal rolled).
  if (isLiveSkillUnlockOffer(player, offer)) {
    const activeNow = countActiveSkills(player);
