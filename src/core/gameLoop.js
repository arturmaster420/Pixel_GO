...
function render() {
  ...
  function renderPersistentHubCoreOverlay(ctx, state) {
    try {
...
function syncPersistentRunUnlocks(state) {
  ...
  for (const key of ["rockets", "energyBomb", "fireBomb", "iceBomb"]) {
    const hasSkill = ((p.runSkills?.[key] | 0) > 0) || !!p.runEvolutions?.[`${key}Fusion`] || (key === "rockets" && !!p.runEvolutions?.rocketFusion) || (key === "energyBomb" && !!p.runEvolutions?.energyBombFusion) || (key === "fireBomb" && !!p.runEvolutions?.fireBombFusion) || (key === "iceBomb" && !!p.runEvolutions?.iceBombFusion);
...
function openFloorShopReplaceOverlay(state, player, pick) {
  ...
  showFloorShopOverlay({
    ...
    onPickCb: (rep) => {
      ...
      const res = performFloorShopPurchase(state, player, String(livePick.id || ''), replaceKey);
...
function openFloorShopOverlay(state) {
  ...
  showFloorShopOverlay({
    ...
    onPickCb: (pick) => {
      ...
      const res = performFloorShopPurchase(state, p, String(livePick.id || ''), null);
      if (!res?.ok) {
        if (res?.reason === 'need_replace' || res?.reason === 'bad_replace' || getLiveReplaceRequirement(p, livePick)?.needsReplace) {
          if (openFloorShopReplaceOverlay(state, p, { ...livePick, _replaceReq: getLiveReplaceRequirement(p, livePick) })) return;
        }
...
function performFloorShopPurchase(state, buyer, offerId, replaceKey) {
  ...
  if (offer) {
    const evoUnlockByOfferKey = {
      rocketFusion: 'rockets',
      rockets: 'rockets',
      energyBombFusion: 'energyBomb',
      energyBomb: 'energyBomb',
      fireBombFusion: 'fireBomb',
      fireBomb: 'fireBomb',
      iceBombFusion: 'iceBomb',
      iceBomb: 'iceBomb',
    };
    const unlockKey = evoUnlockByOfferKey[String(offer.key || '')] || '';
    if (unlockKey) {
      try {
...
// Compatibility bridge: older multiplayer/joiner paths may still reference
// maybeEnterOnlineDeathOverlay(). Keep this alias so refactors in overlays do not
// hard-crash LAN joiners if one stale call slips through during active development.
function maybeEnterOnlineDeathOverlay(state) {
  return maybeEnterDeathContinueOverlay(state);
}
