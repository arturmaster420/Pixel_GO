...
import { rollFloorShopOffersStandard, rollFloorShopOffers, describeFloorShopOffer, tryBuyFloorShopOfferEx, getFloorShopRerollCost, rerollFloorShopOffersForPlayer, offerNeedsReplace, isLiveSkillUnlockOffer } from "./floorShop.js";
...
function openFloorShopReplaceOverlay(state, player, pick) {
  ...
  if (!(isLiveSkillUnlockOffer(player, livePick) && needsReplace)) return false;
...
function openFloorShopOverlay(state) {
  ...
  showFloorShopOverlay({
    ...
    onPickCb: (pick) => {
      ...
      if (!res?.ok) {
        if (res?.reason === 'need_replace' || res?.reason === 'bad_replace') {
          if (openFloorShopReplaceOverlay(state, p, livePick)) return;
        }
...
function applySnapshotToClient(state, snap) {
  ...
  const recentPstate = (typeof state._netLastAppliedPStateAt === "number" && st != null)
    ? Math.abs(st - state._netLastAppliedPStateAt) <= 0.12
    : false;
