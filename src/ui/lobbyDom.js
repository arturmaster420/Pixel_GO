...
export function initLobbyDom(game) {
  ...
  el.btnCopyInvite?.addEventListener("click", async () => {
    ...
    const relay = getRelayUrlOverride();
    const url = relay
      ? `${location.origin}/?code=${encodeURIComponent(room)}&ws=${encodeURIComponent(relay)}`
      : `${location.origin}/?code=${encodeURIComponent(room)}`;
    try {
