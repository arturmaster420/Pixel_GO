...
// Buff icons row (right-bottom)
  ...
  // Pixel_GO: Breach patch hint (SAS-like)
  if (!state.paused) {
    const bh = state._breachHint;
    const flash = typeof state._breachPatchedFlash === 'number' ? state._breachPatchedFlash : 0;
    if ((bh && bh.canPatch) || flash > 0) {
      const boxW = 320;
      const boxH = 34;
      const bx = (scaledW - boxW) * 0.5;
      const by = scaledH - boxH - 18;
      ctx.fillStyle = flash > 0 ? `rgba(80,160,255,${0.18 + 0.22 * clamp01(flash / 0.6)})` : "rgba(0,0,0,0.35)";
      ...
      if (flash > 0.01) {
        txt = "Breach patched!";
      } else if (bh && (bh.patchedLeft || 0) > 0.05) {
        txt = `Patched (${(bh.patchedLeft || 0).toFixed(1)}s)`;
      } else {
        txt = "E: Patch breach";
      }
      ctx.fillStyle = "rgba(255,255,255,0.92)";
      ...
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText(txt, bx + boxW * 0.5, by + boxH * 0.5);
      ctx.textBaseline = "alphabetic";
    }
  }
