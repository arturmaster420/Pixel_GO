...
export function renderHUD(ctx, state) {
  ...
  fillRoundedRect(ctx, stripX, stripY, stripW, stripH, 14, 'rgba(9,13,20,0.84)', 'rgba(255,255,255,0.12)', 1);
  fillRoundedRect(ctx, stripX + 2, stripY + 2, Math.max(24, stripW * 0.22), 4, 2, accent, null, 0);

  const fullWallet = stripW >= 700;
  ...
  const walletW = drawCompactEssenceWalletInline(ctx, progression, 0, 0, { compact: !fullWallet, draw: false });

  let rightCursor = stripInnerRight;

  if (walletW > 0) {
    const walletX = rightCursor - walletW;
    drawCompactEssenceWalletInline(ctx, progression, walletX, chipY + Math.max(0, Math.floor((chipH - 16) * 0.5)), { compact: !fullWallet, draw: true });
