...
function renderHeroSkillCard(entry, opts = {}) {
  ...
  const button = make('button', {
    type: 'button',
    title: String(opts.title || `${entry?.name || 'Card'} • ${biomeMeta.label}`),
    style: {
  ...
  button.appendChild(make('div', { text: biomeMeta.short, style: {
    position: 'absolute', left: '10px', top: '10px', fontSize: '10px', fontWeight: '900', color: '#fff0cf', textTransform: 'uppercase', letterSpacing: '0.08em',
  } }));
...
function renderSelectedSkillCardShowcase(entry, activeHero) {
  if (!entry) return null;
  const accent = getFrameAccent(entry?.race || 'mecha');
  const biomeMeta = getCardBiomeMeta(entry);
  const evoLine = getCardEvolutionLine(entry);
  const box = make('div', { style: {
    padding: '14px', borderRadius: '18px', border: `1px solid ${accent}66`,
    background: `linear-gradient(180deg, ${accent}18, rgba(18,18,26,0.98) 32%, rgba(10,12,18,0.98))`,
    boxShadow: `0 0 0 1px ${accent}22 inset`, display: 'grid', gridTemplateColumns: '122px 1fr', gap: '14px', alignItems: 'stretch',
  } });
  box.appendChild(renderHeroSkillCard(entry, { selected: true, width: 122, height: 188, title: `${entry.name} • ${biomeMeta.label}` }));
  const right = make('div', { style: { display: 'flex', flexDirection: 'column', gap: '10px' } });
  right.appendChild(make('div', { text: String(entry.name || 'Card'), style: { fontSize: '20px', fontWeight: '900', color: '#fff1ce', letterSpacing: '0.03em' } }));
  right.appendChild(make('div', { text: `${entry.kind === 'skill' ? 'Skill Card' : 'Passive Card'} • ${biomeMeta.label} • ${'★'.repeat(Math.max(1, Number(entry.currentStars || 1) || 1))}`, style: { fontSize: '12px', color: 'rgba(238,245,255,0.84)', textTransform: 'uppercase', letterSpacing: '0.10em' } }));
  const chipWrap = make('div', { style: { display: 'flex', gap: '8px', flexWrap: 'wrap' } });
  chipWrap.appendChild(renderCharacterChip('Level', `${Math.max(1, Number(entry.level || 1) || 1)}`, `${accent}44`));
  chipWrap.appendChild(renderCharacterChip('Copies', `${Math.max(0, Number(entry.copiesOwned || 0) || 0)}`, 'rgba(255,216,122,0.24)'));
  chipWrap.appendChild(renderCharacterChip('Biome', biomeMeta.short, `${accent}44`));
  chipWrap.appendChild(renderCharacterChip('Hero', String(activeHero?.name || 'Hero'), `${getFrameAccent(activeHero?.race || 'mecha')}33`));
  right.appendChild(chipWrap);
  right.appendChild(make('div', { text: evoLine, style: { fontSize: '12px', lineHeight: '1.48', color: 'rgba(232,245,255,0.88)' } }));
  if (Array.isArray(entry?.tags) && entry.tags.length) right.appendChild(make('div', { text: `Focus: ${entry.tags.join(' • ')}`, style: { fontSize: '12px', lineHeight: '1.42', color: 'rgba(255,235,188,0.88)' } }));
  if (Array.isArray(entry?.routeTags) && entry.routeTags.length) right.appendChild(make('div', { text: `Route: ${entry.routeTags.join(' • ')}`, style: { fontSize: '12px', lineHeight: '1.42', color: 'rgba(210,230,255,0.82)' } }));
...
function renderHeroSelectedCardSummary(state, prog, entry, activeHero) {
  if (!entry) return renderSimpleInfoCard('Selected Card', ['Pick a card from the archive or deck.'], { accent: 'rgba(143,216,255,0.26)' });
  const accent = getFrameAccent(entry.race || 'mecha');
  const loadout = getHeroLoadoutActionPreview(prog, entry.cardId, activeHero);
  const shell = make('div', { style: { display: 'flex', flexDirection: 'column', gap: '10px', padding: '12px', borderRadius: '18px', border: `1px solid ${accent}44`, background: 'linear-gradient(180deg, rgba(18,22,33,0.98), rgba(9,12,18,0.98))', boxShadow: `0 0 0 1px ${accent}18 inset` } });
  shell.appendChild(renderSelectedSkillCardShowcase(entry, activeHero));
  const quick = make('div', { style: { display: 'flex', flexWrap: 'wrap', gap: '8px' } });
  quick.appendChild(renderCharacterChip('Stars', entry.starsText || '★', `${accent}44`));
  quick.appendChild(renderCharacterChip('Lv', `${Math.max(1, entry.level | 0)}`, 'rgba(143,216,255,0.24)'));
  quick.appendChild(renderCharacterChip('Copies', `${Math.max(0, entry.copiesOwned | 0)}`, 'rgba(255,216,122,0.22)'));
  if (entry?.isEquipped) quick.appendChild(renderCharacterChip('Deck', 'EQUIPPED', `${accent}3a`));
  shell.appendChild(quick);
  const btnRow = make('div', { style: { display: 'grid', gridTemplateColumns: '1fr', gap: '8px' } });
  ...
  const btn = make('button', { className: 'btn', type: 'button', text: btnText, style: { padding: '10px 12px', opacity: (loadout?.canEquip || loadout?.canUnequip) ? '1' : '0.6' } });
...
function renderModernSkillDeckStage(state, prog, activeHero, selectedEntry, replaceFlow = null, pendingEntry = null) {
  ...
  stage.appendChild(make('div', { text: (ESSENCE_META[heroBiome] || ESSENCE_META.mecha).short, style: { position: 'absolute', left: '50%', bottom: '12px', transform: 'translateX(-50%)', padding: '5px 10px', borderRadius: '999px', background: `${accent}26`, border: `1px solid ${accent}55`, color: '#f4fbff', fontSize: '10px', fontWeight: '800' } }));
  return stage;
}
...
function renderModernSkillCard(state, prog, activeHero, entry, selected = false, opts = {}) {
  ...
  card.addEventListener('click', () => selectCharacterEntry(state, { kind: 'card', key: entry.cardId }));
  ...
  raceMark.appendChild(make('div', { text: (ESSENCE_META[entry.race] || ESSENCE_META.mecha).short.toUpperCase(), style: { fontSize: '8px', fontWeight: '900', color: '#f7eddc', textShadow: '0 1px 1px rgba(0,0,0,0.45)' } }));
...
function renderSkillReplacePanel(state, prog, activeHero, incomingEntry, targetEntry, targetSlotIndex = -1) {
  ...
  function detail(entry, title, accent) {
    ...
    if (entry) {
      box.appendChild(make('div', { text: `${entry.name} • ${(ESSENCE_META[entry.race] || ESSENCE_META.mecha).label}`, style: { fontSize: '12px', fontWeight: '800', color: '#f4fbff' } }));
...
function renderSkillsPhoneLandscapeTab(state, prog) {
  try {
    ...
    const heroBiome = String(activeHero?.race || 'mecha');
    const accent = getFrameAccent(heroBiome);
    const isPhoneLandscape = window.innerWidth > window.innerHeight && window.innerHeight <= 540;
    const stageWidth = isPhoneLandscape ? 146 : 156;
    const detailWidth = isPhoneLandscape ? 274 : 300;
    const cardsPerPage = isPhoneLandscape ? 12 : 12;
    const cardWidth = isPhoneLandscape ? 88 : 96;
    const cardHeight = isPhoneLandscape ? 118 : 126;
    const view = getHeroCardViewState(state);
    ...
    const pageInfo = paginateHeroCards(state, filtered, cardsPerPage);

    const wrap = make('div', { style: { display: 'grid', gridTemplateColumns: `${stageWidth}px minmax(0, 1fr) ${detailWidth}px`, gap: '10px', height: '100%', minHeight: '0', alignItems: 'stretch' } });
    const stageWrap = make('div', { style: { minHeight: '0', overflow: 'hidden' } });
    stageWrap.appendChild(renderModernSkillDeckStage(state, prog, activeHero, selectedEntry, effectiveReplaceFlow, pendingEntry));
    wrap.appendChild(stageWrap);

    const center = make('div', { style: { display: 'flex', flexDirection: 'column', gap: '8px', minHeight: '0', overflow: 'hidden' } });
    const filterRow = make('div', { style: { display: 'flex', justifyContent: 'space-between', gap: '8px', flexWrap: 'wrap', alignItems: 'center', padding: '8px 10px', borderRadius: '14px', border: '1px solid rgba(255,216,122,0.14)', background: 'linear-gradient(180deg, rgba(16,19,28,0.98), rgba(8,11,18,0.96))', opacity: pendingEntry ? '0.35' : '1', pointerEvents: pendingEntry ? 'none' : 'auto' } });
    const leftFilters = make('div', { style: { display: 'flex', gap: '6px', flexWrap: 'wrap' } });
    [['all','All Skills'],['hero','Hero Biome'],['equipped','Equipped']].forEach(([mode,label]) => {
      const active = mode === 'hero' ? (view.raceFilter === 'hero' && view.kindFilter !== 'equipped') : (mode === 'equipped' ? view.kindFilter === 'equipped' : (view.kindFilter !== 'equipped' && view.raceFilter !== 'hero'));
      const btn = make('button', { className: 'btn', type: 'button', text: label, style: { padding: '6px 10px', borderRadius: '999px', borderColor: active ? 'rgba(255,216,122,0.52)' : 'rgba(255,255,255,0.12)', background: active ? 'linear-gradient(180deg, rgba(112,78,18,0.98), rgba(58,42,12,0.98))' : 'linear-gradient(180deg, rgba(23,28,42,0.98), rgba(12,16,24,0.98))', color: active ? '#fff0c8' : '#d8e5ff', fontSize: '11px' } });
      btn.addEventListener('click', () => {
        if (mode === 'hero') setHeroCardViewState(state, { kindFilter: 'skill', raceFilter: 'hero' });
        else if (mode === 'equipped') setHeroCardViewState(state, { kindFilter: 'equipped', raceFilter: 'all' });
        else setHeroCardViewState(state, { kindFilter: 'skill', raceFilter: 'all' });
      ...
      leftFilters.appendChild(btn);
    });
    filterRow.appendChild(leftFilters);
    filterRow.appendChild(make('div', { text: `${Math.max(0, filtered.length | 0)} cards • page ${Math.max(1, pageInfo.page + 1)}/${Math.max(1, pageInfo.pages)}`, style: { fontSize: '11px', color: 'rgba(214,232,255,0.68)' } }));
    ...
    const gridWrap = make('div', { style: { display: 'flex', flexDirection: 'column', gap: '6px', minHeight: '0', padding: '8px', borderRadius: '16px', border: '1px solid rgba(255,216,122,0.14)', background: 'linear-gradient(180deg, rgba(16,19,28,0.98), rgba(8,11,18,0.96))', overflow: 'hidden', flex: '1 1 auto', opacity: pendingEntry ? '0.22' : '1', pointerEvents: pendingEntry ? 'none' : 'auto' } });
    const grid = make('div', { style: { display: 'grid', gridTemplateColumns: `repeat(6, ${cardWidth}px)`, gridTemplateRows: `repeat(2, ${cardHeight}px)`, gridAutoRows: `${cardHeight}px`, columnGap: '4px', rowGap: '4px', minHeight: '0', alignContent: 'start', justifyContent: 'start' } });
    for (const entry of pageInfo.items) {
      try {
        grid.appendChild(renderModernSkillCard(state, prog, getActiveHero(prog), entry, selectedEntry?.cardId === entry.cardId, {
          width: cardWidth,
          height: cardHeight,
          iconSize: isPhoneLandscape ? 22 : 24,
          titleSize: isPhoneLandscape ? 8 : 8,
          buttonSize: isPhoneLandscape ? 8 : 9,
        }));
      } catch (err) {
        const failCard = make('div', { text: String(entry?.name || 'Card'), style: { height: `${cardHeight}px`, borderRadius: '8px', border: '1px solid rgba(255,120,120,0.25)', background: 'rgba(22,14,16,0.95)', color: '#fff0d4', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '6px', textAlign: 'center', fontSize: '10px' } });
        grid.appendChild(failCard);
      }
    }
    if (!pageInfo.items.length) grid.appendChild(make('div', { text: 'No skills in this view.', style: { gridColumn: '1 / -1', padding: '40px 0', textAlign: 'center', color: 'rgba(214,232,255,0.46)' } }));
    gridWrap.appendChild(grid);
    gridWrap.appendChild(renderHeroGalleryPager(state, pageInfo));
    center.appendChild(gridWrap);
    wrap.appendChild(center);

    const right = make('div', { style: { display: 'flex', flexDirection: 'column', gap: '8px', minHeight: '0', overflowY: 'auto', paddingRight: '4px' } });
    const header = make('div', { style: { padding: '12px 14px', borderRadius: '18px', border: `1px solid ${accent}44`, background: 'linear-gradient(180deg, rgba(18,22,33,0.98), rgba(9,12,18,0.98))', boxShadow: `0 0 0 1px ${accent}18 inset` } });
    header.appendChild(make('div', { text: 'Selected Card', style: { fontSize: '14px', fontWeight: '900', color: '#fff1cb', textTransform: 'uppercase', letterSpacing: '0.05em' } }));
    header.appendChild(make('div', { text: 'Card details stay here on the right so the archive grid can stay compact on phone-landscape.', style: { marginTop: '5px', fontSize: '11px', lineHeight: '1.38', color: 'rgba(214,232,255,0.74)' } }));
    ...
    const safeGrid = make('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(6, 96px)', columnGap: '4px', rowGap: '4px', justifyContent: 'start' } });
    for (const entry of safeCards) {
      safeGrid.appendChild(make('div', { text: String(entry?.name || 'Skill'), style: { minHeight: '64px', borderRadius: '8px', border: '1px solid rgba(255,216,122,0.18)', background: 'rgba(14,18,28,0.96)', color: '#fff4d6', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '6px', textAlign: 'center', fontSize: '10px' } }));
    }
    fail.appendChild(safeGrid);
    return fail;
  }
}
...
export function renderCharacterMenu(state, force = false) {
  ...
  if (el.characterPanel) {
    ...
    el.characterPanel.style.height = 'calc(100vh - 82px)';
    el.characterPanel.style.maxHeight = 'calc(100vh - 82px)';
  ...
  if (el.characterPanel && anchor) {
    try {
      const viewportW = Math.max(1, window.innerWidth || document.documentElement?.clientWidth || 1);
      const xPct = Math.max(6, Math.min(94, (Number(anchor.x || 0) / viewportW) * 100));
      el.characterPanel.style.transformOrigin = `${xPct}% calc(100% + 36px)`;
      if (typeof el.characterPanel.animate === 'function') {
        el.characterPanel.animate([
          { transform: 'translateY(26px) scale(0.965)', opacity: 0.72 },
