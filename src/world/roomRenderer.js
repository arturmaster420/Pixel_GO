...
function drawStationLuxuryTrim(ctx, x, y, w, h, { isBridge = false, isHub = false, pid = '', time = 0 } = {}) {
  if (!(w > 0 && h > 0)) return;
  const majorX = w >= h;
  const cut = isBridge ? Math.min(12, Math.min(w, h) * 0.18) : Math.min(20, Math.min(w, h) * 0.18);
  ...
  ctx.shadowBlur = isBridge ? 16 : 24;
  ctx.shadowOffsetY = isBridge ? 8 : 12;
...
function drawCosmosScreen(ctx, state) {
  ...
  if (isNeutralSpace) {
    const planetCx = w * 0.82;
    const planetCy = h * 1.16;
    const planetR = Math.max(w, h) * 0.54;
    const planet = ctx.createRadialGradient(planetCx - planetR * 0.18, planetCy - planetR * 0.24, 0, planetCx, planetCy, planetR);
    planet.addColorStop(0, 'rgba(178,214,255,0.82)');
    planet.addColorStop(0.24, 'rgba(118,168,238,0.78)');
    planet.addColorStop(0.58, 'rgba(52,84,148,0.72)');
    ...
    atmosphere.addColorStop(0.82, 'rgba(146,220,255,0.16)');
    ...
    orbitalHaze.addColorStop(0.62, 'rgba(92,154,255,0.06)');
...
function isLuxurySpaceRoom(room, arenaSpec) {
  const biomeKey = String(room?.biomeKey || '').toLowerCase();
  const visualPreset = String(arenaSpec?.visualPreset || '').toLowerCase();
...
function drawOrbitalBackdropBehindRoom(ctx, room, { x0, y0, x1, y1, w, h }, isHub = false) {
  const cx = x0 + w * (isHub ? 0.72 : 0.80);
  const cy = y0 + h * (isHub ? 1.10 : 0.98);
  const r = Math.max(w, h) * (isHub ? 1.10 : 0.98);
  ctx.save();
  const limb = ctx.createRadialGradient(cx, cy, r * 0.68, cx, cy, r);
  limb.addColorStop(0, isHub ? 'rgba(42,92,182,0.08)' : 'rgba(56,116,212,0.10)');
  limb.addColorStop(0.72, isHub ? 'rgba(44,112,220,0.16)' : 'rgba(72,156,255,0.24)');
  limb.addColorStop(0.88, isHub ? 'rgba(188,228,255,0.32)' : 'rgba(212,242,255,0.42)');
  limb.addColorStop(0.93, isHub ? 'rgba(138,226,255,0.26)' : 'rgba(136,218,255,0.34)');
...
function drawTile(ctx, room, cam, { alpha = 1, fall = 0, label = "", time = 0, state = null } = {}) {
  ...
  if (luxurySpace) {
    drawOrbitalBackdropBehindRoom(ctx, room, { x0, y0, x1, y1, w, h }, !!arenaSpec?.rules?.isHub || (room.index | 0) === 0);
  }
  ...
  // Top face: legacy slab for fallback rooms, geometry-only rooms skip square body.
  {
    ...
    const cachedArt = luxurySpace ? getRoomStaticArtCache(state, room, arenaSpec, hue) : null;
    if (cachedArt) {
      ...
      if (!String((room && room.biomeKey) || '').toLowerCase() || arenaSpec?.rules?.isHub || String((room && room.biomeKey) || '').toLowerCase() === 'neutral') {
        drawNeutralSpaceSurfaceFX(ctx, room, { x0, x1, y0, y1, w, h }, { hue, time });
      }
