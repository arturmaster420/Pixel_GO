...
// ---- Breaches (holes in the wall rails) ----

function drawBreaches(ctx, room, geo, { hue = 210, time = 0 } = {}) {
  const brs = room && Array.isArray(room.breaches) ? room.breaches : null;
  ...
  const len = clamp(room.side * 0.14, 92, 152);
  const depth = clamp(room.side * 0.055, 36, 58);
  const inset = 10;

  for (const b of brs) {
    if (!b) continue;
    const patchedLeft = (b.patchedLeft || 0);
    const pulse = 0.5 + 0.5 * Math.sin(time * 3.2 + (b.id ? b.id.length : 0));
    ...
    if (b.side === 'W') {
      ...
      // N is currently unused; keep a sensible fallback.
      rx = (b.x) - len * 0.5;
      ry = y0 + inset;
      rw = len;
      rh = depth;
    ...
    // Shadow under the breach frame
    ctx.fillStyle = 'rgba(0,0,0,0.18)';
    ctx.fillRect(rx + 2, ry + 3, rw, rh);

    // Frame (metal)
    const frameG = ctx.createLinearGradient(rx, ry, rx + rw, ry + rh);
    frameG.addColorStop(0, 'rgba(20,24,30,0.85)');
    frameG.addColorStop(1, 'rgba(8,10,14,0.92)');
    ...
    const cx = rx + rw * 0.5;
    const cy = ry + rh * 0.5;
    const rr = Math.max(rw, rh) * 0.7;
    const portal = ctx.createRadialGradient(cx, cy, 0, cx, cy, rr);
    const glowHue = (hue + 120) % 360;
    portal.addColorStop(0, hsla(glowHue, 95, 60, patchedLeft > 0 ? 0.06 : (0.22 + 0.12 * pulse)));
    portal.addColorStop(0.45, 'rgba(0,0,0,0.92)');
    portal.addColorStop(1, 'rgba(0,0,0,1)');
    ctx.fillStyle = portal;
    ctx.fillRect(rx + 3, ry + 3, rw - 6, rh - 6);

    // Emissive rim
    ctx.strokeStyle = hsla(glowHue, 95, 62, patchedLeft > 0 ? 0.10 : (0.32 + 0.18 * pulse));
    ctx.lineWidth = 4;
    ctx.strokeRect(rx + 2.5, ry + 2.5, rw - 5, rh - 5);

    // If patched: draw a sci-fi plate over the breach (temporary)
    if (patchedLeft > 0.02) {
      const plateG = ctx.createLinearGradient(rx, ry, rx + rw, ry + rh);
      plateG.addColorStop(0, 'rgba(110,120,130,0.92)');
      plateG.addColorStop(1, 'rgba(70,78,86,0.96)');
      ctx.fillStyle = plateG;
      ctx.fillRect(rx + 6, ry + 6, rw - 12, rh - 12);

      // Bolts
      ctx.fillStyle = 'rgba(0,0,0,0.35)';
      const bx = [rx + 10, rx + rw - 10];
      const by = [ry + 10, ry + rh - 10];
      for (const xx of bx) {
        for (const yy of by) {
          ctx.beginPath();
      ...
      // Timer bar (tiny)
      const p = clamp(patchedLeft / 10.0, 0, 1);
      ctx.fillStyle = 'rgba(0,0,0,0.28)';
      ctx.fillRect(rx + 10, ry + rh - 14, rw - 20, 6);
      ctx.fillStyle = hsla(glowHue, 95, 60, 0.55);
      ctx.fillRect(rx + 10, ry + rh - 14, (rw - 20) * p, 6);
    } else {
      // Alien "arrival" flicker
      const gg = ctx.createRadialGradient(cx, cy, 0, cx, cy, rr * 1.25);
      gg.addColorStop(0, hsla(glowHue, 95, 62, 0.10 + 0.08 * pulse));
      gg.addColorStop(1, 'rgba(0,0,0,0)');
      ctx.fillStyle = gg;
      ctx.beginPath();
      ctx.arc(cx, cy, rr * 1.25, 0, Math.PI * 2);
      ctx.fill();
    }
  }
}

function drawTile(ctx, room, cam, { alpha = 1, fall = 0, label = "", time = 0 } = {}) {
  const b = room.bounds;
  const x0 = b.minX;
  const x1 = b.maxX;
  ...
  // Top face: "cosmic platform" surface (panels + subtle emissive lines)
  {
    ...
    // Breaches: alien "holes" in the wall rails (SAS-like spawns)
    drawBreaches(ctx, room, { x0, x1, y0, y1, w, h, fall }, { hue, time });
...
export function renderRoomsBackground(ctx, state) {
  ...
  for (const it of rooms) {
    ...
    drawTile(ctx, r, cam, { alpha: a, fall, label, time: state.time || 0 });
  }
