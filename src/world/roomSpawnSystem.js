...
function wrapClampUpdate(enemy, bounds, pad = 10) {
  ...
  enemy.update = (self, dt, state) => {
    // Entrance animation from a breach ("crawl in from outside")
    const ent = self && self._enterFromBreach;
    if (ent && ent.t < 1) {
      ent.t += dt / (ent.dur || 0.55);
      if (ent.t > 1) ent.t = 1;
      const k = ent.t;
      // easeOutCubic
      const e = 1 - Math.pow(1 - k, 3);
      self.x = ent.ox + (ent.ix - ent.ox) * e;
      self.y = ent.oy + (ent.iy - ent.oy) * e;
      // Don't run AI while entering.
    } else {
      orig(self, dt, state);
    }
    // Clamp to room bounds (skip while entering so we can start outside the wall)
    if (ent && ent.t < 1) return;
    const b = bounds;
...
function pickBreachSpawn(room, rd, players, minDist = 260) {
  ...
  const op = rd.getBreachOuterPoint(room, pick.b, 90);
  return {
    breachId: pick.b.id,
    side: pick.b.side,
    inside: pick.ip,
...
export class RoomSpawnSystem {
  ...
  update(dt) {
    ...
    for (let i = 0; i < batch; i++) {
      ...
      const pos = brPick ? brPick.inside : pickSpawnPos(bounds, players, spawnMinDist, 14);
      ...
      // Spawn from breach: start outside and slide in.
      if (brPick) {
        enemy._spawnBreachId = brPick.breachId;
        enemy._enterFromBreach = {
          t: 0,
          dur: 0.55,
          ox: brPick.outside.x,
          oy: brPick.outside.y,
        ...
        enemy.x = brPick.outside.x;
        enemy.y = brPick.outside.y;
      }
