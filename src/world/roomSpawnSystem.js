...
function applyEnemyEncounterBehavior(enemy, room, choice, tuning) {
  ...
  const hpMul = clamp(Number(tuning?.hpMul || 1) * (enemy.isElite ? 1.06 : 1), 0.84, 1.35);
  const speedMul = clamp(Number(tuning?.speedMul || 1) * (enemy.isElite ? 1.01 : 1), 0.88, 1.22);
  const damageMul = clamp(Number(tuning?.damageMul || 1) * (enemy.isElite ? 1.04 : 1), 0.90, 1.26);
...
export class RoomSpawnSystem {
  ...
  onRoomChanged(room) {
    ...
    if (this.roomIndex <= 0) {
      ...
      const base = 4 + floorNo * 1.45 + roomOrd * 1.4 + (this.isBossRoom ? 3.2 : 0);
      this.quotaTotal = Math.max(4, Math.round(base * partyScale * this.roomDifficultyScale * (ep.quotaMul || 1)));
      this.aliveCap = clamp(Math.round(4 + floorNo * 0.55 + (this.isBossRoom ? 2 : 0) + (ep.aliveAdd || 0)), 3, 16);
      this.wavesTotal = this.isBossRoom ? clamp(2 + Math.floor(floorNo / 4), 2, 4) : clamp(1 + Math.floor((floorNo - 1) / 4) + (ep.waveDelta || 0), 1, 4);
  ...
  update(dt) {
    ...
    const zone = 1 + Math.floor((floorNo - 1) / 3);
    const ep = this._encounterProfile || getEncounterProfile(room);
    const minDist = clamp((200 + floorNo * 8) * (ep.minDistMul || 1), 160, 320);
    const spawnInterval = clamp((0.92 - floorNo * 0.018) * (ep.spawnIntervalMul || 1), 0.30, 1.05);
    this._spawnTimer += dt;
    if (this._spawnTimer < spawnInterval) return;
    this._spawnTimer = 0;

    const batch = Math.min(remainingInWave, freeSlots, this.isBossRoom ? 3 : (this._encounterProfile?.spawnBatchCap || 4));
    for (let i = 0; i < batch; i++) {
      ...
      const eliteChance = clamp((0.04 + floorNo * 0.01) * (ep.eliteChanceMul || 1), 0.03, 0.55);
      ...
      const useBiome = biomeKey && Math.random() < (ep.biomeChance || 0.55);
      ...
      applyEnemyEncounterBehavior(enemy, room, choice, ep);
