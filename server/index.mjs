...
wss.on("error", (err) => {
  ...
  if (code === 'EADDRINUSE') {
    console.error(`[COOP] Port ${PORT} is already in use. Stop the old relay server or start with another PORT, for example: PORT=8081 npm run server`);
    process.exit(1);
    return;
  }
  console.error('[COOP] WS relay server failed:', err);
  process.exit(1);
});
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (data) => {
    ...
    if (msg.type === "host") {
      ...
      room.clients.set(clientId, { ws, nickname: client.nickname, avatarIndex: client.avatarIndex||0, auraId: client.auraId||0, ready: !!client.ready, meta: msg.meta || null, lastSeen: client.lastSeen });
    ...
    if (msg.type === "join") {
      ...
      if (host) {
        send(host.ws, { type: "playerJoined", playerId: clientId, nickname: client.nickname });
        if (msg.meta) send(host.ws, { type: "syncMeta", from: clientId, meta: msg.meta || null, phase: "joinHandshake" });
      }
      return;
    }
    ...
    if (msg.type === "fastJoin") {
      ...
      if (room) {
        ...
        if (host) {
          send(host.ws, { type: "playerJoined", playerId: clientId, nickname: client.nickname });
          if (msg.meta) send(host.ws, { type: "syncMeta", from: clientId, meta: msg.meta || null, phase: "fastJoinHandshake" });
        }
        return;
      }
      ...
      newRoom.clients.set(clientId, { ws, nickname: client.nickname, avatarIndex: client.avatarIndex||0, auraId: client.auraId||0, ready: !!client.ready, meta: msg.meta || null, lastSeen: client.lastSeen });
...
if (msg.type === "input") {
    ...
    if (msg.type === "respawn") {
      // Forward death-flow / respawn request to host only.
      if (!room.hostId) return;
      const host = room.clients.get(room.hostId);
      if (!host) return;
      send(host.ws, { type: "respawn", from: clientId, payload: msg.payload || null, meta: msg.meta || null });
      return;
    }
...
if (msg.type === "runRequest") {
  ...
  return;
    ...
    if (msg.type === "progGain") {
      // Only host can credit persistent progression resources to a joiner.
      if (clientId !== room.hostId) return;
      const to = (msg.to != null ? String(msg.to) : "");
      ...
      const target = room.clients.get(to);
      if (!target) return;
      send(target.ws, { type: "progGain", to, payload });
      return;
    }
